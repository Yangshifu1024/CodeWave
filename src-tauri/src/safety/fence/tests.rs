...
fn plan_readonly_gh_in_pipeline_still_gated() {
    ...
    let p = plan_policy();
    assert_eq!(
        run_policy("gh pr list | head -3", &ws, &roots, p),
        Verdict::Allow
    );
...
fn plan_readonly_gh_readonly_tables_have_no_write_entries() {
    ...
    for word in PLAN_READONLY_GH_WORDS {
        assert!(
            !WRITE_SUBCOMMANDS.contains(word),
            "只读表混入写子命令：{word}"
        );
    }
}
...
fn t21_l3_string_content_masked() {
    ...
    assert_eq!(
        run(
            "git commit -m \"fix git push --force handling\"",
            &ws,
            &roots
        ),
        Verdict::Allow
...
fn t43_git_non_reset_unaffected() {
    ...
    assert_eq!(
        run(
            "git commit -m \"revert after git reset --hard\"",
            &ws,
            &roots
        ),
        Verdict::Allow
...
fn cmd_fallback_caret_escape_defeated() {
    let (ws, _o, roots) = fixture();
    expect_confirm(
        &run("ta^skkill /f /im app.exe", &ws, &roots),
        "taskkill via caret",
    );
}
...
fn cmd_fallback_ampersand_chain_blocked_and_confirm() {
    ...
    expect_block(&run("echo a & del /q x.txt", &ws, &roots), "del in chain");
    expect_block(
...
fn cmd_host_c_recursion_inner_danger() {
    ...
    expect_confirm(
        &run("cmd /c git reset --hard", &ws, &roots),
        "cmd /c git reset",
    );
    // 无害内层照常放行
...
fn fish_function_body_danger_blocked() {
    let (ws, _o, roots) = fixture();
    expect_block(
        &run("function cleanup; rm -rf build; end", &ws, &roots),
        "fish function rm",
    );
    expect_confirm(
...
fn fish_persistence_entry_confirm() {
    let (ws, _o, roots) = fixture();
    expect_confirm(
        &run("source ~/.config/fish/config.fish", &ws, &roots),
        "source",
    );
...
fn fish_harmless_readonly_allowed() {
    ...
    assert_eq!(
        run("function greet; echo hello; end", &ws, &roots),
        Verdict::Allow
    );
}
...
fn wsl_inner_danger_recursive() {
    ...
    // bash -c 内层删除 → Block
    expect_block(
        &run("wsl bash -c \"rm -rf /\"", &ws, &roots),
        "wsl bash -c rm",
    );
    // sh -c 内层删除 → Block
    expect_block(
        &run("wsl sh -c 'rm -rf build'", &ws, &roots),
        "wsl sh -c rm",
    );
    // bash -c 内层灾难 → Confirm
    expect_confirm(
...
fn wsl_exec_form_inner_danger() {
    ...
    expect_confirm(
        &run("wsl -e shutdown -h now", &ws, &roots),
        "wsl -e shutdown",
    );
...
fn wsl_unrecognized_form_conservative_confirm() {
    let (ws, _o, roots) = fixture();
    expect_confirm(
        &run("wsl.exe --list --verbose", &ws, &roots),
        "wsl.exe --list",
    );
...
fn impw03_family_more_confirm() {
    ...
    expect_confirm_hr(
        run("uncompress c.txt.Z", &ws, &roots),
        "uncompress",
        "原文件",
    );
...
fn l0_file_verify_plan_allowed() {
    ...
    let p = plan_policy();
    assert_eq!(
        run_policy("sha256sum a.bin", &ws, &roots, p),
        Verdict::Allow
    );
    assert_eq!(
    ...
    assert_eq!(
        run_policy("cmp a.txt b.txt", &ws, &roots, p),
        Verdict::Allow
    );
}
...
fn l2_ln_inside_allowed() {
    let (ws, _o, roots) = fixture();
    assert_eq!(run("ln -s in.txt link.txt", &ws, &roots), Verdict::Allow);
}
...
fn l2_rsync_outside_create_confirm() {
    ...
    let v = run(
        &format!("rsync -a in.txt {}", target.display()),
        &ws,
        &roots,
    );
    assert!(
...
fn l2_curl_big_o_allowed() {
    let (ws, _o, roots) = fixture();
    assert_eq!(
        run("curl -O https://example.com/f.txt", &ws, &roots),
        Verdict::Allow
    );
}
...
fn l3_persistence_confirm() {
    ...
    expect_confirm_hr(
        run("systemctl restart nginx", &ws, &roots),
        "systemctl",
        "确认",
    );
...
fn rw01_sed_in_place_confirm() {
    let (ws, _o, roots) = fixture();
    expect_confirm_hr(run("sed -i 's/a/b/' f.txt", &ws, &roots), "sed -i", "原地");
    expect_confirm_hr(
...
fn rw02_sed_in_place_plan_block() {
    let (ws, _o, roots) = fixture();
    let v = check_command_policy("sed -i 's/a/b/' f.txt", ws.path(), &roots, plan_policy());
...
fn rw04_tar_plan_semantics() {
    ...
    assert_eq!(run_policy("tar -tzf a.tgz", &ws, &roots, p), Verdict::Allow);
}
...
fn rw05_tar_create_targets() {
    ...
    let v = run(&format!("tar -czf {} f.txt", target.display()), &ws, &roots);
    assert!(
        matches!(v, Verdict::Confirm(ConfirmReason::OutsideCreate(_))),
        "got {v:?}"
    );
...
fn rw06_unzip_semantics() {
    ...
    let p = plan_policy();
    assert_eq!(run_policy("unzip -l a.zip", &ws, &roots, p), Verdict::Allow);
}
