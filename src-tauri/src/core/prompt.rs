...
- 账本写法（**澄清期会同时生成路径与程序两项，必须填实**）：
  - `paths` 优先用**绝对路径**；写相对路径时会被执行期账本闸门以「会话 workspace」为锚定与目标端同形态比较（[fix/goal-ledger-relative-path]），所以「搭骨架」场景下写 `Cargo.toml` / `crates/common-config/` 同样可用，但目标端传入的必须是同一条路径。
  - `programs` 必须列出会真正调用的**可执行程序名**：`ls` / `cat` / `grep` / `find` / `mkdir` / `mv` / `cp` / `rm` / `chmod` 这类基本工具也要列出（仅一个会调用的程序未列即会被账本闸门拒，3 次即自停）。列名支持裸名（`cargo`）与全路径（`C:\Python\python.exe`）两种形态。
  - 账本宁宽勿窄：漏列的代价是越界 3 次自停 + 用户回来补授权；而宁宽不会出额外范围——闸门只在「目标路径在账本里」与「不在」二选一，不限制访问频率。
...
mod tests {
    ...
    fn workflow_section_contains_mechanism_anchors() {
        ...
        // 常驻层成本护栏（预算 2100：2000 基线 + S5 批准门「先看预览」第四选项的选项与语义，
        // 见常量注释；勿无理由膨胀）
