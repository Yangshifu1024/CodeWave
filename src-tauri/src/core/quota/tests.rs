...
fn heal_quarantines_unrecognised_shapes() {
    let shapes: Vec<(&str, Vec<u8>)> = vec![
        ...
        ("verified 类型错（字符串）", br#"{"verified": "x"}"#.to_vec()),
...
fn heal_accepts_numeric_current_version_and_sanitizes() {
    for (label, version) in [
        ...
        let healed = QuotaState::load(dir.path());
        assert_eq!(healed.version, state::VERSION, "{label}：写回统一落当前版本");
        assert_eq!(healed.verified, vec![record("p-keep", "2026-01-01T00:00:00Z")]);
    }
}
...
fn quarantine_and_load_summaries_never_echo_file_content() {
    ...
    let failure = state::parse_state(shape.as_bytes()).expect_err("verified 不是数组应判形态不识别");
    ...
    assert!(!structured.contains(&long), "摘要回显了文件内容：{structured}");
