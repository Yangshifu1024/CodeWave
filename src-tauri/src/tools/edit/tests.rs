...
async fn crlf_file_edited_with_lf_oldtext_roundtrip() {
    ...
    let out = EditTool
        .run(
            &ctx,
            serde_json::json!({"files":[{"path":"c.txt","changes":[
                {"oldText":"one\ntwo","newText":"uno\ndos"}]}]}),
        )
        .await;
    assert!(out.ok, "{out:?}");
...
async fn fuzzy_match_warning_surfaces_in_outcome() {
    ...
    let out = EditTool
        .run(
            &ctx,
            serde_json::json!({"files":[{"path":"fz.txt","changes":[
                {"oldText":"if x {\n    a();\n}","newText":"if x {\n    b();\n}"}]}]}),
        )
        .await;
    assert!(out.ok, "{out:?}");
    assert!(
        out.warnings.iter().any(|w| w.contains("缩进平移归一")),
        "{:?}",
        out.warnings
    );
...
async fn concurrent_runtimes_editing_same_file_serialize() {
    ...
    let t1 = tokio::spawn(async move {
        EditTool
            .run(
                &ctx1,
                serde_json::json!({"files":[{"path":"r.txt","changes":[
...
fn schema_and_description_document_files_shape() {
    ...
    assert!(
        d.contains("files 是数组") && d.contains("path"),
        "description 必须含形状句：{d}"
    );
}
...
fn salvage_args_only_touches_unambiguous_shapes() {
    // 正常形态：不触碰
    let normal =
        serde_json::json!({"files":[{"path":"a","changes":[{"oldText":"x","newText":"y"}]}]});
...
async fn e_args_error_includes_expected_structure_example() {
    ...
    assert!(
        err.message.contains("files 数组每个元素内"),
        "{:?}",
        err.message
    );
    ...
    let out2 = tool
        .run(
            &ctx,
            serde_json::json!({"path":"doc.md","files":[
...
async fn flat_single_file_args_salvaged_and_applied() {
    ...
    let out = EditTool
        .run(
            &ctx,
            serde_json::json!({"path":"doc.md","changes":[
...
async fn files_as_object_salvaged_and_applied() {
    ...
    let out = EditTool
        .run(
            &ctx,
            serde_json::json!({"files":{"path":"doc.md","changes":[
...
async fn edit_puts_post_write_check_into_data_per_file() {
    ...
    let model_text = crate::tools::compact::compact_for_model(
        crate::tools::ToolKind::FileWrite,
        "edit",
        &out,
        true,
    );
