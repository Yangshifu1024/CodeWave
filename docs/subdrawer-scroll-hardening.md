...
抽屉里的「分配的任务」块改为**默认收起**：标签行成为开关（右侧 chevron，展开时旋转 90°），收起态显示单行 ellipsis 预览气泡（hover 有完整 title），点击标签行或预览气泡均可展开/收起，`aria-expanded` 同步。切换子代理时重置回收起。动机：任务文本是静态上下文（且动辄数屏），过程流才是抽屉的活内容——收起后首屏直接让给过程流。浏览器 harness 实测：收起预览单行 35px、展开完整气泡 590px、aria 状态正确；`pnpm --dir ui test` 256/256 全绿。
