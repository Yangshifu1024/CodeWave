...
| `pnpm --dir ui test` | **464 passed / 57 文件**（新增：i18n 键对称、数据目录行与会话段不存在、折叠默认展开与持久化、编辑器下拉四态、额度多提供商（摘要/展开/数值行/风险类与进度条填充元素/更新时间位置/无凭证/失败/可见性门控）+ 展示纯函数、栏宽纯函数、分隔条交互（拖动/键盘/双击/失焦清理/夹取期禁用）、`--rb-width` 真接线、偏好读盘容错） |
