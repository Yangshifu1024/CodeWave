...
| D2 | 展示位置与形态 | 命中率**同一段**内联追加 ` · 18.2 tok/s`（默认单行，窄窗口折行见 [composer-toolbar-context-hit-rate](./composer-toolbar-context-hit-rate.md) §9）；悬停 tooltip 给明细（本轮平均速率、TTFT、输出 tokens、生成耗时、工具等待合计）。不新增控件/区域 |
...
- 速率段插在 `ctx-label` 的命中段之后（同一段，默认单行；窄窗口按 §9 折行），**无数据不渲染**：`breakdown` 缺失时仍是逐字的 `上下文 —`（既有断言零改动）。
...
8. 暗色主题 + 切英文 + 拉窄窗口 → 新段与命中段同色同字号；窄窗口在「命中」「速率」前折行（两段各自完整、不错位，见 [composer-toolbar-context-hit-rate](./composer-toolbar-context-hit-rate.md) §9）；tooltip 文案英文侧完整。
