...
- 2026-09-19 · [chat-loading-indicator.md](./chat-loading-indicator.md) — 聊天流式等待指示改用 antd 加载图标（需求与方案，**待实施**）：`<LoadingOutlined spin />` 替代行内方块光标与 `.cursor` 闪烁动画（渲染点 `ChatMessages.tsx` 与 `SubagentDrawer.tsx`，样式在 `theme/app.css`）；同类「运行中」指示（会话行 / 压缩中 / 子代理卡）已用同款图标，本次向既有惯例统一；顺带清理易误伤的全局 `.cursor` 与 `blink` 关键帧；保持「至多一个流式项」不变量与光标落在文本末尾的位置语义
