...
- 2026-09-24 · [preview-skill.md](./preview-skill.md) — 新增内置技能 **preview**（方案可视化预览：方案概览 / 界面示意，自包含 HTML 经 `render_html` 产出，形态与降级纪律写在技能正文）+ **批准门第三选项「先看预览」**（plan 档 P3 与标准工作流 S5 口径一致，三处提示词联动；`WORKFLOW_SECTION` 常驻层预算 2000 → 2100）。**切档安全是本批关键**：ConfirmEach 档的切档是 `arch 闸标记 || 有效应答`，而标准工作流批准门带 `switchToAutoEdit=true`、形状又是标准批准闸 → **`arch_flag` 单独就能触发切换**，只排除有效应答不够；故新增 `preview_only`（仅在批准形询问里识别 `id="preview"`）并在调用点 `!preview_only_answer` **同时封掉两条通道**（后端 `has_valid_answer` 语义不改），前端 `AskPanel` 同步（预览-only 不切胶囊档位 + 预览项点击即直提）；预览项识别为 **id（大小写不敏感）+ label 兜底**，且从**批准候选**（`approved_hit` 集合与 `approve_option_id`）中剔除——否则它的 label 含「执行方案」时前端会单边改档；配套 10 个后端用例（含「选批准仍切档」对照、批准候选剔除、id 大小写·label 兜底、§3.4 补丁与「非闸形状仍轻量切档」守护）与 5 个前端用例。**另附相邻修复**：完整批准路径从 `switch && (Plan || arch_flag)` 收紧为 `switch && (Plan || (arch_flag && approved)) && (!gate_shape || approved)`（`gate_shape` = 单题 + 含 `id="approve"`）——此前 ConfirmEach 档的 arch 闸标记/有效应答都能开启切换，于是在完整流水线批准门选「补充意见」会被当成方案批准（冻结基线 + 注入「立即执行」+ `plan_approved`）；现在批准闸上只有真选中批准项才动档位（连轻量切档也不触发），非闸形状询问的既有语义不变
...
- 2026-09-20 · [quota-state-heal.md](./quota-state-heal.md) — `quota.json` 启动期自愈：损坏（语法坏 / 顶层非对象 / `verified` 非数组）→ 移动隔离为 `.corrupt` 且不重建；无效记录（空白 id、空串/非法 RFC3339 时间戳）清理；元素级类型错逐元素丢弃而非整文件隔离；`version` 高于当前 schema 完全不碰；核心不变式「自愈后要么文件不存在、要么严格语义必定读得回来」（为此把「形态不可读」也纳入重写判据）