...
- 越界计数 `ledger_denials` 在单次无人值守运行中累计到 3 → 判为账本漂移，自停出报告（收尾成因 `LedgerDrift`，优先于通用 `Blocked` 呈现）；显式续跑重置本轮计数，保留被拦历史（恢复方案见 [goal-mode-recovery](./goal-mode-recovery.md)）。
...
- 用户切走档位 = 目标自动进入「已暂停」（不锁档）；切回目标档不自动续跑，需点「继续推进」。暂停态也可点「修订目标」退回只读澄清期，修改合同并重新批准。
...
- **临时会话 / 计划任务**：任务 runtime 仍显式 `FullAccess`；主会话持久化目标档活动标记、模型与思考力度，前档快照及其它权限档重启时回落全局默认，详见 [goal-mode-recovery](./goal-mode-recovery.md) 和 [session-model-recovery](./session-model-recovery.md)。
...
- 账本执行期不可扩展：漏列程序/目录会导致越界拒绝（本轮累计三次自停）。澄清期提示块要求列出会真正调用的**可执行程序名**与改动目录；被拒项进入 `blocked`。暂停后可用「修订目标」退回只读澄清期修改合同并重新批准，详见 [goal-mode-recovery](./goal-mode-recovery.md)。
