...
### 2.2 澄清阶段（只读）
- 工具集 = 只读工具 + `ask` + 只读子代理；**排除写工具与 `command`/`service`/`scheduled_task`**——「澄清不动任何东西」是机制保证，不是提示词自律。
...
- **命令**：程序名过账本；需确认级命令（工作区内写 / 工作区外新建）免确认但携带的写目标路径也要过账本。**父问题因此不可能出现**：任何带写目标的命令都被路径白名单兜住。
...
### 2.6 暂停 / 续跑 / 收尾
- 用户点停止 = **暂停**：状态置 `paused`、目标与进度全保留；「继续推进」按钮（Composer 提示条 + 右栏目标段）调 `resume_goal` 起一轮新 run（`status` 回 `executing`；起跑失败自动回滚为 `paused`）。该命令是**可重放**的，故 host 与 core 两层都校验「当前档位必须是目标模式」（`E_GOAL_MODE_REQUIRED`）——否则会出现「档位已切走、目标却回到执行中而闸门不生效」的静默空窗。
