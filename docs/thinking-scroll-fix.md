...
§3.1 第 1 项引入的「目标值比对」有实现缺陷：`progTarget` 取的是**未钳的 `scrollHeight`**，而 `scrollTop` 会被
浏览器钳到 `scrollHeight - clientHeight`，两者差值恒等于 `clientHeight` ⇒ 豁免永不成立；叠加「`scroll` 事件在
下一帧才派发、期间内容又长了一截」，自家跳底会被判成「用户滚走了」，跟随被关掉且不自愈（新会话不跟随 /
提问后要手动点「回到底部」）。修法与回归用例见 [chat-autoscroll-regression-fix](./chat-autoscroll-regression-fix.md)。

另：同批修正了「切 Tab / 首次激活的贴底分支未登记豁免窗口」的同类缺口。
