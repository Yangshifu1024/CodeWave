...
| 4 | 排布 | 上下文**一行内**追加（`· 阈 60% · 命中 78%`）+ 模型按钮改「供应商 / 模型」 | 阈值与用量/窗口同属「占用事实」；命中率独立成段以便单独隐藏 |
...
1. 新开临时会话未发消息：工具条只显示「上下文 —」，无阈值/命中段、不换行。
...
8. 拖窄窗口 + 暗色主题 + 切英文：工具条不换行；模型名省略但 hover 是全名；四档颜色可读。
