# preview 技能 + 批准门第三选项「先看预览」

> 2026-09-24 · 新增内置技能 `preview`，并在两处方案批准门（plan 档 P3 / 标准工作流 S5）加入「先看预览」选项。
...
| 位置 | 改动 |
|---|---|
| `src-tauri/src/core/prompt.rs` 的 `WORKFLOW_SECTION`（S5 批准门） | 选项增为三个：`approve`（批准开发）/ `revise`（补充意见）/ `preview`（先看预览）；写明选 preview = 不批准也不驳回：先加载 preview 技能渲染预览，再重发同一询问（不计入修订轮次）。`WORKFLOW_SECTION` 有常驻层成本断言，预算 2000 → 2100 并补理由注释 |
...
- 前端同步（`ui/src/features/tools/AskPanel.tsx`）：`submitWith` 的 `anyAnswer`（ConfirmEach 下触发胶囊切档）排除「预览-only」；`pickAndSubmitIfApprove` 扩展为「批准项或预览项点击即提交」——三选项的批准门里，让用户为「先看预览」再点一次「提交回答」纯属多余。
...
| `src-tauri/src/core/prompt.rs` | S5 批准门三选项与语义；常驻层预算 2000 → 2100 + 注释 |
