...
3. ask 批准协议：方案 ask 选「执行方案」→ 会话切执行档并注入执行指令（后续批次扩为**三选项**：两个批准类选项各带 `mode` 声明目标档位，切档目标不再硬编码，见 [docs/mode-gate-and-subagent-sync](./mode-gate-and-subagent-sync.md)）。
...
- **US3 批准与执行**：选「执行方案」（或「完全访问执行」）后严格按已确认 todos 执行，完成后按范围决定是否强制 code-reviewer。AC：执行完成后多文件/跨层变更必须出现 reviewer 调用。
...
| P3 技术方案与计划 | 技术方案（文件级改动点/风险/回滚；git 仓库内拟定分支名 `<type>/<slug>`，slug ≤24 字符、基线当前 HEAD，非 git 仓库注明跳过）+ plan todos 登记（含验证项）+ 验证方式 | todos 非空且含验证项 → 才允许发起批准 ask；批准询问题干与 plan 文本列明分支名 | 用户（ask：**执行方案 / 完全访问执行 / 补充意见**——前两项各带 `mode` 声明目标档位，批准 = 用户在两档中选一（自动编辑 / 完全访问）+ 预授权创建并切换该分支，[docs/mode-gate-and-subagent-sync](./mode-gate-and-subagent-sync.md)） |
...
批准命中（批准类选项 = 携带 `mode` 的选项；旧的 id/label 宽松子串匹配作为兼容兜底保留）且当前 Plan 档时，依次校验，任一不过**批准不生效**（不切档、不注入）：
