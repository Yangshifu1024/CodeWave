...
- **历史落盘体积与图片外置**（[docs/session-history-limits](./docs/session-history-limits.md)）：8MB 上限量的是**单会话历史 gzip 后**字节——纯文本够不到（trim 到 256k token ≈ 300KB gz），瓶颈全在图片（base64 近似不可压缩，且只按 1600 token/张计、几乎不受 trim 约束）。**附件图片不再进历史文件**：落盘走独立 DTO `PersistedMessage`/`PersistedContent`（`ImageInline`/`ImageBlob` 两态），blob 存 `sessions/<owner>.imgblob/<sha256 前 32 hex>`（内容寻址去重），`load_history` 读回 base64 后才交上层——**内存与 wire 零改动**（勿改 `core/types.rs` 的 `Content`，那会撞三协议字节断言与 match 穷尽点）。越限阶梯 = 剥图 → `trim(.., 1)` 只保最后一轮 → 才拒存；**状态挂 `SessionMeta.history_status`**（索引独立于历史文件，所以历史写失败也能留痕、重启后仍可见）+ `run:done` 载荷可选字段 `history_save`（当场可见）。保存路径必须持 `save_lock`（锁序 `save_lock → index_lock`）——GC 用的是内存快照，不串行化会让并发保存互删对方仍引用的图；子代理 blob owner 是 `<父>__<sub>`（子 id 只有 32 bit，裸用会跨会话撞目录）
