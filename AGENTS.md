...
- **批准门 = 四选项显式选档**（[docs/mode-gate-and-subagent-sync](./docs/mode-gate-and-subagent-sync.md)、[docs/preview-skill](./docs/preview-skill.md)）：ask 选项带 `mode: Option<ApprovalMode>`（serde default + `skip_serializing_if`，`None` 时 wire 上不出现该键）声明目标档位；批准类判定 = `mode.is_some()`（`id="approve"` 与 `switchToAutoEdit` 锚点保留为兼容兜底，有测试钉死）；切档目标取**用户所选选项的 `mode`**（可从逐项确认档/计划档一次跨到完全访问档）；第四项 `preview`（先看预览）**不带 mode**，语义是不批准也不驳回（渲染预览后重发同一询问）。**预览项必须在三处都被排除**（少一处即「前端切档、后端不切」的单边静默提权）：结构化通道（不带 mode → `selected_target_mode` 返 `None`；前端 `planPath` / `modePath` 另乘 `!previewOnly`）、宽松批准匹配（`approved_hit` 与前端同款匹配都要先剔除预览项 id——模型自拟 id 含 `approve` 时会误命中）、轻量切档（`valid_answer` / `switch` 叠加 `!preview_only_answer`）。**反面纪律**：`approval_shape` 是「无安全语义」的宽松渲染标记，**不得**收紧成剔除预览项（`preview_option_ids` 以它为前置，收紧反而会打开 `valid_answer` 通道静默抬档）；`plan_approval_gate`（G2 分析产物 / G3 todos 基线）不变；`ask:opened` 只增字段、事件键名不动
...
- **目标模式（[完整交付契约](./docs/goal-mode-delivery-rebuild.md)，第五档 `ApprovalMode::Goal`）**：澄清只读 + ask；仅 Executing 开放执行工具，权限复用 FullAccess，不再使用账本白名单。用户显式选择预算后一次批准；预算累计主/子代理与压缩用量。完成必须绑定批准的机器检查命令、实际成功调用和当前源文件指纹，并通过独立审查；人工项进入 awaiting_acceptance，由用户确认。停止进入 stopping，子代理及自建服务排空后才 paused；普通暂停消息仅补充，显式续跑。G2 保留、G3 关闭；前档快照留在 SessionRuntime::goal_prev_mode，推进指令不入历史；旧 ledger 字段只作存量数据。
- **会话恢复的完整出参备份**（[docs/session-restore-fidelity](./docs/session-restore-fidelity.md)）：历史里只有**模型侧瘦身文本**，因此「解析不了」的出参（头尾截断、失败 `[error …]` 前缀）会由批次层额外落一份**整套 `ToolOutcome`** 到 `~/.codewave/sessions/<会话>.toolres/<call_id>.json`，恢复时经 IPC `load_tool_outcomes` 批量回填工具卡。键必须是 **provider 侧 `tool_use.id`**——实时 `ctx.call_key` 是批内随机 `batch:index`、不可持久，而前端恢复时的 `callKey` 正是前者；它**永不参与出网**（不改 `rt.history`、不改工具出参 JSON、不增计费），随会话级联删除，且**不进**右栏「文件」面板（那里只读产物登记边车）。前端**三个消费方共用一套回填**（判据 `lossyToolKeys` 与 `restoredToolData` 同源 → `backfillToolOutcomes`）：主会话工具卡、子代理卡（合成 key `restored:<callKey>` 在回填后**改名**成真实 `sub_id`，抽屉据此才能拉到真实过程流、`report` 同时补全）、以及 `openSubDrawer` 拉回的子代理过程流内工具卡

- **会话历史落盘：分段 append-only JSONL**（[docs/session-history-storage](./docs/session-history-storage.md)、[docs/session-history-limits](./docs/session-history-limits.md)）：落盘是 `histories/<id>/0001.jsonl…`（明文不压缩、每个段首行是 header、段尾可能有一条 seal；水位边车 `.segmeta.json`），**不再 trim、也没有 8MB 上限**——落盘保留完整转录（压缩只记 `compaction` 边界，`wire` 由边界重建并守 256k；`display` 全量不裁，前端按段分页）。**任何 fallback（前缀哈希不匹配 → 基线段）只新增段，绝不覆盖/截断/删除既有段**——这是「不丢历史」的底线。段封口 = 条数 200 或 512KB 先触者，且**绝不切在 tool_use/tool_result 配对中间**；加载丢弃尾部残行、坏行跳过、坏段跳过并提示，**绝不因容错报「历史损坏」**。体积靠 `HISTORY_SOFT_WARN_BYTES`(200MB) / `HISTORY_HARD_FUSE_BYTES`(1GB)：软线照常写只提示，硬线**只停写、绝不删数据**且可自愈；状态仍挂 `SessionMeta.history_status` + `run:done.history_save`，且**新状态必须同步纳入 `SaveReport::is_clean()`**（`drive.rs` 的上报判据是 `filter(|r| !r.is_clean())`，漏改则提示永远发不出去）。**wire 装载必须有界**（从尾部读到够用即停，与「全量 + repair + trim」逐字节对拍），打开会话不得随历史总量线性增长。旧 `.json.gz` 读兼容不迁移，且**设置页清理入口的判据必须与读路径同源**（`segments::has_readable_messages`）——**无新格式可读数据时绝不删旧文件**（那是唯一副本）。**图片外置**：落盘走独立 DTO `PersistedMessage`/`PersistedContent`（`ImageInline`/`ImageBlob` 两态），blob 存 `sessions/<owner>.imgblob/<sha256 前 32 hex>`（内容寻址去重），`load_history` 读回 base64 后才交上层——**内存与 wire 零改动**（勿改 `core/types.rs` 的 `Content`，那会撞三协议字节断言与 match 穷尽点）。保存路径必须持 `save_lock`（锁序 `save_lock → index_lock`）——GC 用的是内存快照，不串行化会让并发保存互删对方仍引用的图；**GC 的引用集合是「磁盘上所有保留段引用的 blob 并集」**；子代理 blob owner 是 `<父>__<sub>`（子 id 只有 32 bit，裸用会跨会话撞目录）
