export default {
  ...
  settings: {
    ...
    validation: "写入后语义校验",
    validationHint: "探测对象是各语言的「语言服务器」可执行文件（如 gopls、rust-analyzer），不是语言本身；每行右侧另标语言工具链是否就绪。命令覆盖留空 = 自动探测，未找到时可在本行安装。",
    ...
    validationLangJson: "JSON（内置解析，非 LSP）",
    lspCommandPh: "留空 = 自动探测",
    lspFound: "已找到", lspFoundVersion: "已找到 v{{version}}", lspMissing: "未找到", lspDisabled: "已关闭",
    lspServerMissing: "语言服务器未安装",
    lspServerMissingNoSdk: "语言服务器未安装（且未找到 {{name}}）",
    lspSdkReady: "{{name}} 已就绪", lspSdkMissing: "未找到 {{name}}",
    lspInstall: "安装", lspInstallDone: "安装完成", lspInstallFailed: "安装失败",
    lspNeedRuntime: "需先安装 {{name}}", lspDownload: "下载", lspManualInstall: "手动安装",
    lspBudget: "全局预算",
    lspSyncWindow: "写后等待诊断（毫秒）", lspMaxDiagnostics: "单次诊断条数上限", lspMaxChars: "单次诊断字符上限",
    lspIdleTtl: "server 闲置回收（毫秒）", lspMaxServers: "单项目并发 server 上限",
    lspMaxFileBytes: "超过该体积跳过（字节）", lspDedupeLimit: "同一诊断重复回喂上限",
    lspDiscovery: "发现",
  ...
  lsp: {
    installTitle: "未找到 {{server}}",
    installCommand: "安装命令：{{command}}",
    install: "安装",
    installing: "安装中…",
    installDone: "已安装：{{server}}",
    installFailed: "安装失败",
    manualTitle: "需要手动安装 {{server}}",
  ...
  rightbar: {
    ...
    // 订阅额度段（[docs/rightbar-info-refactor-and-subscription-quota]）
    quota: "订阅额度", quotaRefresh: "刷新额度", quotaLoading: "读取中…", quotaRetry: "重试",
    ...
    quotaSource: "凭证来源：{{source}}",
    quotaNoCredential: "未检测到订阅凭证（悬浮查看配置位置）",
    quotaInvalid: "凭证形态不可用：{{reason}}", quotaFailed: "额度请求失败：{{reason}}",
    window: {
